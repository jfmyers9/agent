...
export function isNativeCompactionDetails(value: unknown): value is NativeCompactionDetails {
	...
	if (!isNativeCompactionIdentity(value)) {
		return false;
	}
	...
	return (
		...
		(details.compactResponseId === undefined || isNonEmptyString(details.compactResponseId)) &&
		(details.requestMeta === undefined || isNativeCompactionRequestMeta(details.requestMeta))
	);
}
