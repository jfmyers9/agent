...
import { type ResponsesInputItem, serializeMessagesToCompactRequest } from "./serializer";
...
function cloneOpaqueWindow(window: readonly unknown[]): ResponsesInputItem[] {
...
async function handleSessionBeforeCompact(event: SessionBeforeCompactEvent, piContext: ExtensionContext) {
	...
	if (latestNativeCompaction.ok) {
		...
		request = {
			...
			input: [
				...cloneOpaqueWindow(details.compactedWindow),
		...
		const sessionContext = (
			piContext.sessionManager as typeof piContext.sessionManager & {
